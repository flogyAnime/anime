...
def episode(request, slug):
    ...
    render_ = 'episode.html'
    redirect_ = f'/anime-list/{slug}/e'
    ...
    this_episode = Episode_animetitans.objects.get(slug=slug)
    context = {
        # basic info
        'title': f'Flagy | {this_episode.anime} E{this_episode.number}',
        'description': f'مشاهدة وتحميل الحقة {this_episode.number} من انمي {this_episode.anime} مترجة بجودة عالية Flagy',
        'css': ['episode.css', 'anime_info.css'],
        'js': ['episode/main.js',],
        ...
        # other
        'this_anime': Animes_animetitans.objects.get(title=this_episode.anime),
        'this_episode': this_episode,
        'this_episodes': Episode_animetitans.objects.filter(anime=this_episode.anime),
