...
def Comments_main(request, slug):
    render_ = 'comments.html'
    redirect_ = f'/anime-list/{slug}/comments'
    this_ep = Episode_animetitans.objects.get(slug=slug)
    ...
    context = {
        ...
        # others
        'Comments': Comments.objects.filter(episode=this_ep).order_by('-added_date')[:100],
        'Comments_pin': Comments.objects.filter(pin=True).order_by('-added_date')[:5],
    ...
    # post request
        ...
        # comments
        # like
        if request.POST.get('comment-id') and request.POST.get('like') == 'True':
            comments_id = Comments.objects.get(comments_id=request.POST.get('comment-id'))
            if profile in comments_id.like.all():
                comments_id.like.remove(profile)
            else:
                comments_id.like.add(profile)
            comments_id.dislike.remove(profile)
            return redirect(redirect_)

        # dislike
        if request.POST.get('comment-id') and request.POST.get('dislike') == 'True':
            comments_id = Comments.objects.get(comments_id=request.POST.get('comment-id'))
            if profile in comments_id.dislike.all():
                comments_id.dislike.remove(profile)
            else:
                comments_id.dislike.add(profile)
            comments_id.like.remove(profile)
            return redirect(redirect_)

        # add comment
        if request.POST.get('user-comment'):
            comment = request.POST.get('user-comment')
            print(comment[-8:])
            print('#')
            while True:
                comments_id = str(randint(0,1000000000000000000000000)).zfill(25)
                if comments_id != Comments.comments_id:  # 25
                    break
            if request.user.username == "000000000000000000000000" and comment[-8:] == 'pin True':
                comment = comment[:-8]
                Comments(comments_id=comments_id,comment=comment, user=profile, episode=this_ep, pin=True).save()
            else :
                Comments(comments_id=comments_id,comment=comment, user=profile, episode=this_ep).save()
            return redirect(redirect_)

        # add reply
        if request.POST.get('user-replys') != 'True':
            comment = request.POST.get('user-comment')
            while True:
                comments_id = str(randint(0,1000000000000000000000000)).zfill(25)
                if comments_id != Comments.comments_id:  # 25
